...
int main(){
    ...
    checked(cudaMalloc(&state,32*sizeof(float)));checked(cudaMemset(state,0,32*sizeof(float)));
    ...
    checked(cudaMemset(damageMask,0,4096*4096));checked(cudaMalloc(&meta,16));checked(cudaMemset(meta,0,16));checked(cudaMalloc(&mining,256));
