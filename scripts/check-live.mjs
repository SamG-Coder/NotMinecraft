...
try{
 ...
 // Exercise fast callbacks independently of the monitor refresh rate.
 // Frame timing is diagnostic: slow hardware must still pass this smoke check.
 ...
 // The app has no fixed frame limit; browser/GPU scheduling determines throughput.
 if(!result.ready||result.error||!result.opaque||errors.length||timing.samples<30||timing.frameLimit!==null)process.exitCode=1;
